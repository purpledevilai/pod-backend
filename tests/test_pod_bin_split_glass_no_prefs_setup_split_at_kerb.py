"""Pod bin system — split-recycling, no prefs, user picks "use Pod yellow, split at kerb".
...
setup options. The user picks: "I'll just put glass in the yellow Pod
with the other recycling and split it out when I empty it."

Pod must then call set_pod_bin_preferences with a yellow Pod slot
description that captures the "mixed recycling in Pod yellow, separate at
the kerb so glass goes to Purple" arrangement.
...
name = "pod_bin_split_glass_no_prefs_setup_split_at_kerb"
description = (
    "User has a Pod system and a 4-bin split-recycling council; pod_bin_preferences "
    "is NOT set. They ask about a glass bottle, then choose to put glass in their "
    "yellow Pod recycling bin alongside other recyclables and separate them at the "
    "kerb when emptying. Pod must show the Purple kerbside bin first, then save "
    "prefs that encode the split-at-kerb arrangement in the yellow Pod slot."
)
    ...
    try:
        ...
        sim = SimAgent(
            ...
            persona=(
                ...
                "glass via your Pod going forward, REPLY: \"I'll just put glass in my "
                "yellow Pod with the rest of the recycling and split it out when I "
                "empty the bin.\" Then, if Pod confirms a setup, briefly acknowledge. "
                "Call end_test ONLY once Pod has confirmed your chosen arrangement "
                "back to you."
            ),
        ...
        target.check_all([
            ...
            AssessTrue(
                ...
                "that captures the arrangement: mixed recycling (including glass) goes "
                "into the yellow Pod, and the user separates glass into the Purple "
                "kerbside bin when emptying"
            ),
            AssessTrue(
                "Pod confirmed the chosen arrangement (Pod yellow as a single bucket, "
                "with the user splitting glass out at the kerb) back to the user in "
                "plain language"
            ),
        ])
